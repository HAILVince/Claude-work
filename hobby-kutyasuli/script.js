/* Hobby Kutyasuli — header state, mobile menu, signup form */
(function () {
  'use strict';
  ...
  /* ---------- booking form ----------------------------------------------
     ...
     school's inbox. No server, no database, nothing to maintain.
  ...
  var validate = function () {
    ...
    [['f-name', 'Add meg a neved.'],
     ['f-issue', 'Írd le, mi az, ami most nem működik.']].forEach(function (p) {
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (DEMO) {
      say('ok', 'Demó mód',
        ...
        'megadott e-mail címre küldi a jelentkezést — a kulcs beállítása után működik.');
      form.reset();
      return;
    }
    ...
    fetch('https://api.web3forms.com/submit', {
      ...
      .then(function (r) {
        ...
        say('ok', 'Megkaptuk',
          'Köszönjük a jelentkezést! Pár napon belül válaszolunk a megadott e-mail címre.');
        form.reset();
        ['f-name', 'f-email', 'f-issue'].forEach(function (id) {
          document.getElementById(id).removeAttribute('aria-invalid');
        });
      })
      .catch(function () {
        say('err', 'Nem sikerült elküldeni',
          'Kérlek próbáld újra egy kicsit később.');
      })
      .then(function () {
