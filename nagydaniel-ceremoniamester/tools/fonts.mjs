/* Self-host the webfonts. Google's CSS serves one file per (weight, subset);
   ...
   latin-ext is not optional here: without it the ő and ű fall back to a
   different font mid-word, which on a Hungarian page is visible in every
   second heading. */
import { writeFileSync, existsSync, mkdirSync } from 'node:fs';

const CSS = 'https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,400;9..144,600'
          + '&family=Inter:wght@400;500;600&display=swap';
const UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 '
         + '(KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
const KEEP = new Set(['latin', 'latin-ext']);
const SLUG = { 'Fraunces': 'fraunces', 'Inter': 'inter' };
