/* Self-host the webfonts. Google's CSS serves one file per (weight, subset);
   several weights can point at the SAME file, so download each unique URL
   once and let the @font-face blocks share it.

   latin-ext is not optional: „Királyleányka" és „Bihardiószeg" nélküle
   szóközépen vált betűtípust. */
import { writeFileSync, existsSync, mkdirSync } from 'node:fs';

const CSS = 'https://fonts.googleapis.com/css2?family=EB+Garamond:wght@400;500;600'
          + '&family=Inter:wght@400;500;600&display=swap';
const UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 '
         + '(KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
const KEEP = new Set(['latin', 'latin-ext']);
const SLUG = { 'EB Garamond': 'ebgaramond', 'Inter': 'inter' };
