...
const B = 'http://127.0.0.1:8193/index.html';
...
const open = async (w, h, d, m) => {
  ...
  // Lazy photos far below the fold are never fetched for a full-page capture.
...
}
{
  ...
  const p = await c.newPage();
  await p.goto(`http://127.0.0.1:8193/screenshots/_sheet.html`, { waitUntil:'networkidle' });
