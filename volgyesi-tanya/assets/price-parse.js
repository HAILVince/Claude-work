/* One parser for termekek.txt, shared by the browser at runtime and by
   tools/bake.mjs at build time, so the list baked into index.html and the
   one rendered from the file can never drift apart.
...
/* A product that has run out stays on the list, greyed, instead of being
   deleted and retyped next week. She writes "elfogyott" in the price
   column; anything else is shown as a price. */
export function isOut(price) {
  return /elfogyott/i.test(String(price));
}
