...
const URL = 'http://127.0.0.1:8190/index.html';
...
/* 1. the real file drives the page */
{
  ...
  is('rows rendered from arak.txt', r.length, 6);
  is('first row', r[0], 'Első alkalom — állapotfelmérés és kezelés (60 perc) = 12 000 Ft');
  is('non-numeric price survives', r.includes('Fizetés = készpénzben, a helyszínen'), true);
  is('group headings', (await page.$$('.ar-g h3')).length, 3);
  await ctx.close();
}

/* 2. an edit shows up */
{
  const { ctx, page } = await open('## Kezelések\nTovábbi kezelés (45 perc) | 10 000 Ft\n');
  const r = await rows(page);
  is('edited price replaces the baked one', r[0], 'További kezelés (45 perc) = 10 000 Ft');
...
for (const [name, body] of [
  ...
  ['pipes deleted', '## Kezelések\nTovábbi kezelés 9 000 Ft\n'],
  ['heading with no items', '## Kezelések\n\n## Bérlet\n'],
  ['random paste', 'kedves tamas\nitt vannak az arak\n'],
]) {
  const { ctx, page } = await open(body);
  const r = await rows(page);
  is(`falls back: ${name}`, r.length, 6);
  await ctx.close();
}
...
{
  const { ctx, page } = await open(
    '## Egyéb\nFizetés | készpénzben a helyszínen, bankkártyát nem tudok fogadni\n');
  const ctx2 = ctx;
...
/* 6. form */
{
  ...
  await page.selectOption('#f-panasz', 'Derékfájás');
  await page.click('#f button[type=submit]');
  ...
  await page.selectOption('#f-panasz', 'Hát, lapockák');
  await page.click('#f button[type=submit]');
