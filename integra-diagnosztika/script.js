...
/* ---------- form ----------------------------------------------------
   ...
   same form posts to a form-to-mail service and lands in her inbox. */
...
form.addEventListener('submit', e => {
  e.preventDefault();
  const nev = form.nev, el = form.elerhetoseg, kor = form.kor;
  [nev, el, kor].forEach(clear);
  ok.hidden = true;
  let bad = null;

  if (!nev.value.trim()) {
    fail(nev, 'Írd be a neved, hogy tudjam, kit keressek.');
    bad ||= nev;
  }
  ...
  if (!kor.value.trim()) {
    fail(kor, 'Az életkor sokat segít, hogy fel tudjak készülni.');
    bad ||= kor;
  }
