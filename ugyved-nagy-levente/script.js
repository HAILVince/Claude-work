/* Dr. Nagy Levente Ákos — header state, mobile menu, contact form */
(function () {
  'use strict';
  ...
  /* ---------- booking form ----------------------------------------------
     ...
     lawyer's inbox. No server, no database, nothing to maintain.
  ...
  var validate = function () {
    var bad = [];
    [['f-name', 'Kérjük, adja meg a nevét.'],
     ['f-desc', 'Kérjük, írja le néhány mondatban, mi történt.']].forEach(function (p) {
    ...
    if (mailBad) bad.push([mail, 'Kérjük, adjon meg egy érvényes e-mail címet.']);

    var gdpr = document.getElementById('f-gdpr');
    if (!gdpr.checked) bad.push([gdpr, 'Az elküldéshez el kell fogadnia az adatkezelést.']);
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (DEMO) {
      say('ok', 'Demó mód',
        ...
        'megadott e-mail címre küldi a megkeresést — a kulcs beállítása után működik.');
      form.reset();
      return;
    }
    ...
    fetch('https://api.web3forms.com/submit', {
      ...
      .then(function (r) {
        ...
        say('ok', 'Megkaptam',
          'Köszönöm a megkeresést. Igyekszem mielőbb válaszolni a megadott elérhetőségen.');
        form.reset();
        ['f-name', 'f-email', 'f-desc'].forEach(function (id) {
          document.getElementById(id).removeAttribute('aria-invalid');
        });
      })
      .catch(function () {
        say('err', 'Nem sikerült elküldeni',
          'Kérjük, próbálja meg újra, vagy hívjon: +36 70 337 0361');
      })
      .then(function () {
