...
form.addEventListener('submit', e => {
  e.preventDefault();
  const nev = form.nev, el = form.elerhetoseg, gep = form.gep, hol = form.telepules;
  [nev, el, gep, hol].forEach(clear);
  ok.hidden = true;
  let bad = null;

  if (!nev.value.trim()) {
    fail(nev, 'Írd be a neved, hogy tudjam, kihez szóljak.');
    bad ||= nev;
  }
  ...
  if (!gep.value) { fail(gep, 'Válaszd ki, milyen gépről van szó.'); bad ||= gep; }
  if (!hol.value.trim()) { fail(hol, 'Írd meg, melyik településen van a gép.'); bad ||= hol; }
